...
bool time_delta_impl::stop()
{
    ...
    d_logger->info("WALL_CLOCK_TIME_DEBUG ({}): Mean = {:.6f} ms, Var = {:.6f} ms",
                    d_name, mean, var);
    return true;
}
...
void time_delta_impl::handle_pdu(pmt::pmt_t pdu)
{
    ...
    if (!pmt::is_real(wct_pmt)) {
        d_logger->debug("PDU received with no wall clock time at {}", t_now);
    } else {
        double pdu_time = pmt::to_double(wct_pmt);
        double time_delta = (t_now - pdu_time) * 1000.0;
        d_logger->debug("{} PDU received at {} with time delta {} milliseconds",
